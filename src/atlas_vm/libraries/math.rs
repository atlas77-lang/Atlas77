...
pub fn abs<'lib>(state: VMState) -> Result<VMData, RuntimeError> {
    let val = state.stack.pop_with_rc(state.object_map)?.as_i64();
    Ok(VMData::new_i64(val.abs()))
}

pub fn pow<'lib>(state: VMState) -> Result<VMData, RuntimeError> {
...
pub fn sqrt<'lib>(state: VMState) -> Result<VMData, RuntimeError> {
    let val = state.stack.pop_with_rc(state.object_map)?.as_f64();
    Ok(VMData::new_f64(val.sqrt()))
}

pub fn min<'lib>(state: VMState) -> Result<VMData, RuntimeError> {
...
pub fn max<'lib>(state: VMState) -> Result<VMData, RuntimeError> {
    let v1 = state.stack.pop_with_rc(state.object_map)?.as_i64();
    let v2 = state.stack.pop_with_rc(state.object_map)?.as_i64();
    Ok(VMData::new_i64(std::cmp::max(v1, v2)))
}

pub fn round<'lib>(state: VMState) -> Result<VMData, RuntimeError> {
    let val = state.stack.pop_with_rc(state.object_map)?.as_f64();
    Ok(VMData::new_i64(val.round() as i64))
}

pub fn random<'lib>(state: VMState) -> Result<VMData, RuntimeError> {
