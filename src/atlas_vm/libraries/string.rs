...
pub fn str_len<'lib>(state: VMState) -> RuntimeResult<VMData<'lib>> {
...
pub fn str_cmp<'lib>(state: VMState) -> RuntimeResult<VMData<'lib>> {
    let string1 = state
        .object_map
...
pub fn trim<'lib>(state: VMState) -> RuntimeResult<VMData<'lib>> {
...
pub fn to_upper<'lib>(state: VMState) -> RuntimeResult<VMData<'lib>> {
    let string_ptr = state.stack.pop_with_rc(state.object_map)?.as_object();
    let raw_string = state.object_map.get(string_ptr)?;
    let string = raw_string.string();
...
pub fn to_lower<'lib>(state: VMState) -> RuntimeResult<VMData<'lib>> {
    let string_ptr = state.stack.pop_with_rc(state.object_map)?.as_object();
    let raw_string = state.object_map.get(string_ptr)?;
    let string = raw_string.string();
...
pub fn split<'lib>(state: VMState) -> RuntimeResult<VMData<'lib>> {
...
pub fn from_chars<'lib>(state: VMState) -> RuntimeResult<VMData<'lib>> {
